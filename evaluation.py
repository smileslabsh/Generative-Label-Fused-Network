...
class LogCollector(object):
    ...
    def tb_log(self, tb_logger, prefix='', step=None):
        ...
        """
        for k, v in self.meters.items():  # self.meters.iteritems():
...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    max_n_word = 0
    for i, (images, image_tag, boxes, captions, lengths, ids) in enumerate(data_loader):
    ...
    for i, (images, image_tag, boxes, captions, lengths, ids) in enumerate(data_loader):
        ...
        img_emb, cap_emb, cap_len = model.forward_emb(images, image_tag, boxes, captions, lengths, volatile=True)
        # print(img_emb)
        if img_embs is None:
            if img_emb.dim() == 3:
            ...
            print(len(data_loader.dataset), max_n_word, cap_emb.size(2))
...
def write_txt(rsum, ar, r, ari, ri, opt):
    txt = []
    txt.append('-----------------------------------------')
    ...
    file_name = 'result_' + modal_name + '.txt'
    parts.write_txt_add(os.path.join(current_path, file_name), txt)
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        current_path = opt.current_path
        file_name = 'Final_onlyBox_' + modal_name + 'PosiAttn_AU_' + str(SPLIT_SIZE1) + 'X' + str(
            BOX_LEN) + '{0}.npy'.format('F30k')
        np.save(os.path.join(current_path, file_name), sims)
        r, rt = i2t(img_embs, cap_embs, cap_lens, sims, opt, return_ranks=True)
        ri, rti = t2i(img_embs, cap_embs, cap_lens, sims, opt, return_ranks=True)
        ar = (r[0] + r[1] + r[2]) / 3
        ...
        write_txt(rsum, ar, r, ari, ri, opt)
        ...
        for i in range(5):
            ...
            print("calculate similarity time:", end - start)
            print("Sims shape:", sims.shape)
            r, rt0 = i2t(img_embs_shard, cap_embs_shard, cap_lens_shard, sims, opt, return_ranks=True, )
            print("Image to text: %.1f, %.1f, %.1f, %.1f, %.1f" % r)
            ri, rti0 = t2i(img_embs_shard, cap_embs_shard, cap_lens_shard, sims, opt, return_ranks=True)
            print("Text to image: %.1f, %.1f, %.1f, %.1f, %.1f" % ri)
        ...
        print("Text to image: %.1f %.1f %.1f %.1f %.1f" %
              mean_metrics[5:10])
    current_path = opt.current_path
    file_path = os.path.join(current_path, 'ranks.pth.tar')
    torch.save({'rt': rt, 'rti': rti}, file_path)
...
def i2t(images, captions, caplens, sims, opt, npts=None, return_ranks=False):
    """
    ...
    """
    current_path = opt.current_path
    sims.dump(os.path.join(current_path, 'sims_i2t.npy'))
    npts = images.shape[0]
    ranks = np.zeros(npts)
    top1 = np.zeros(npts)
...
def t2i(images, captions, caplens, sims, opt, npts=None, return_ranks=False):
    """
    ...
    """
    current_path = opt.current_path
    sims.dump(os.path.join(current_path, 'sims_t2i.npy'))
    npts = images.shape[0]
